...
#   3. `uv sync --all-packages` so the declared extension group and
#      workspace member extras (deerflow-harness's
#      postgres extra in particular) are installed — see PR #2584.
#      `--locked` is intentionally omitted so resolution succeeds against a
#      restricted-network pypi mirror (UV_INDEX_URL); the committed uv.lock
#      targets pypi.org and may not byte-match a mirror index.
#   4. Self-heal: if the first sync fails, recreate .venv and retry once. The
#      retry stays `--locked`-free in the same way; a second failure aborts with
#      recovery instructions rather than starting uvicorn against an
#      environment that does not match the lock.
...
if ! uv sync --all-packages $EXTRAS_FLAGS; then
    ...
    uv venv --clear .venv
    # The retry keeps `--locked` off to allow resolving against the pypi
    # mirror (UV_INDEX_URL) on restricted networks, where the committed
    # uv.lock (locked against pypi.org) does not byte-match the mirror index.
    # A second failure is fatal rather than something uvicorn limps past.
    # `set -e` would already stop here; abort explicitly so the operator gets
    # the fix instead of a bare uv exit code.
    # shellcheck disable=SC2086
    if ! uv sync --all-packages $EXTRAS_FLAGS; then
        echo "[startup] uv sync failed again after recreating .venv." >&2
