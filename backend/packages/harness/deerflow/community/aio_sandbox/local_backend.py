            ...
            if _env_flag_disabled("DEER_FLOW_SANDBOX_IMAGE_STARTUP_CAPS"):
                ...
                cmd.extend(
                    [
                        ...
                        # The shipped image /opt/gem/run.sh also needs FOWNER:
                        # after XDG_RUNTIME_DIR defaults to /run/user/1000 it
                        # chmods that directory before dropping to the gem user
                        # (uid 1000). With CHOWN/SETUID/SETGID/DAC_OVERRIDE but
                        # no FOWNER the chmod fails with EPERM ("Operation not
                        # permitted") and the entrypoint exits 1 before the
                        # readiness endpoint exists — every acquisition then
                        # times out ("Sandbox failed to become ready").
                        "--cap-add=FOWNER",
                        "--security-opt",
                        "no-new-privileges",
                    ]
